...
namespace DTS
{
   ...
   */
   ...
   inline InputStream::InputStream (std::istream & s) : in(s)//(std::ifstream & in)//
   {
      int totalSize = 0;
      int offset16 = 0;
      int offset8 = 0;
      ...
      // Read the header
		int localDTSVersion = 0;
		DTSVersion = 0;

      in.read ((char *) &localDTSVersion, 4) ;//4 changes to 2?? 64 bit OS??  What else 
      in.read ((char *) &totalSize, 4) ;//   is going to have to change below??
      in.read ((char *) &offset16, 4) ;
      in.read ((char *) &offset8, 4) ;
		
		//And... WEIRD!  Reading the value straight to the private int variable BREAKS,
		//but reading to a local value works. (???)
		DTSVersion = localDTSVersion;
